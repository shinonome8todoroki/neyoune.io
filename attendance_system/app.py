...
def index():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            flash(f"記録保存中にエラーが発生しました: {e}", "error")
    ...
    # timestampが文字列の場合、datetimeオブジェクトに変換
...
def admin_dashboard():
    with get_db_connection() as conn:
        records = conn.execute('''
            SELECT users.username, records.action, records.timestamp, records.memo, records.is_deleted
            FROM records
    ...
    return render_template('admin_dashboard.html', records=records, users=users, all_records=records)
...
def all_records():
    ...
    with get_db_connection() as conn:
        total_records = conn.execute('SELECT COUNT(*) FROM records WHERE is_deleted = 0 AND is_private = 0').fetchone()[0]
        records = conn.execute('''
            SELECT users.username, records.id, records.action, records.timestamp, records.memo, records.likes_count
            FROM records JOIN users ON records.user_id = users.id WHERE records.is_deleted = 0 AND users.is_private = 0 ORDER BY records.timestamp DESC LIMIT ? OFFSET ?
